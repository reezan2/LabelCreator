  ...
  async function previewFirst() {
    ...
    if (fmt.key === "small") {
      ...
      const desiHtml = opt.showDesignation
        ? designationLines.slice(0, 3).map((line) => `<span>${escapeHtml(line.text)}</span>`).join("")
        : "";
      ...
      previewEl.innerHTML = `
        <div class="preview-label small-label" style="width:${w}px;height:${h}px;">
          <div class="small-top">
            ...
            <div class="small-title">${desiHtml}</div>
          </div>
          ...
          <div class="barcode-number">${opt.showGencode ? escapeHtml(gencodeText) : ""}</div>
        </div>`;
    } else {
  ...
  async function addOneLabelToDoc(doc, row, detected, fmt, opt) {
    ...
    if (fmt.key === "small") {
      ...
      if (opt.showBarcode && detected.gencode && row[detected.gencode]) {
        ...
        if (barcode) {
          doc.addImage(barcode.url, "PNG", marginX, afterTopY + 0.4, usableWidth, 15.8, undefined, "FAST");
        }
      }
      ...
      if (opt.showGencode && gencodeText) {
        ...
        doc.text(gencodeText, width / 2, height - 2.1, { align: "center" });
      }
      return;
    }
